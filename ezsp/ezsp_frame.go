...
func isValidCallbackID(callbackID byte) bool {
	if isCallbackIDMap[EZSP_ALL_CALLBACKS[0]] == false {
		for _, id := range EZSP_ALL_CALLBACKS {
...
func ezspFrameParse(data []byte) (*EzspFrame, error) {
	...
	if seq-sequence <= 0x80 { /* seq >= sequence */
		return nil, fmt.Errorf("out of sequence recvseq=%d, sequence=%d", seq, sequence)
	}

	if (frmCtrl & 0xE0) != 0x80 {
		return nil, fmt.Errorf("not a valid frame ctrl byte 0x%x", frmCtrl)
	}
	if (frmCtrl & 0x1) != 0 {
		return nil, fmt.Errorf("NCP overflow")
	}
	if (frmCtrl & 0x2) != 0 {
		return nil, fmt.Errorf("NCP truncated")
	}
	if (frmCtrl & 0x4) != 0 {
		common.Log.Debug("NCP callback pending")
	}
	callback := byte((frmCtrl >> 3) & 0x3)
	if callback == 3 {
		return nil, fmt.Errorf("NCP unsupported callback ")
	}
	if callback == 2 {
		common.Log.Debug("NCP asynchronous callback ")
	} else if callback == 1 {
		common.Log.Debug("NCP synchronous callback ")
	} else {
	}
	...
	if isCallbackID && callback == 0 {
		return nil, fmt.Errorf("callback==%d while ID=%s", callback, frameIDToName(frmID))
...
func EzspFrameSend(frmID byte, data []byte) (*EzspFrame, error) {
	...
	// 创建接收回复的ch
	if responseChMap[seq] != nil { //如果上一轮sequence发送时超时，有可能没有close
		close(responseChMap[seq])
		common.Log.Warnf("try to use seq=%d channel map not nil", seq)
	}
